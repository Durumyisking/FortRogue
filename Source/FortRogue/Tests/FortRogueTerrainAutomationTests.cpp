...
bool FFortRogueTerrainMapDefinitionEditTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Blueprint helper summarizes weapon projectile effects"), UFortRogueRewardBlueprintLibrary::GetWeaponEffectSummary(SummaryWeapon).ToString().Contains(TEXT("projectile effect")));
...
bool FFortRogueDestructibleTerrainRuntimeTest::RunTest(const FString& Parameters)
{
	...
	if (StatCharacter)
	{
		...
		UFortRogueWeaponDefinition* DrillWeapon = CreateTestWeaponDefinition(StatCharacter);
		DrillWeapon->Weapon.WeaponTag = FortRogueGameplayTags::Weapon_Shell;
		...
		const FFortRogueShotSpec DrillWeaponShotSpec = StatCharacter->GetCurrentShotSpec();
		TestTrue(TEXT("Weapon projectile effects add shot tags"), DrillWeaponShotSpec.EffectTags.HasTagExact(FortRogueGameplayTags::ShotEffect_Drill));
