...
#include "FRTerrainMapDefinition.generated.h"

class UTexture2D;
class UFRCharacterDefinition;

USTRUCT(BlueprintType)
struct FFRTerrainTextureLayer
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FFREnemyPlacement
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Enemy", meta = (ToolTip = "이 위치에 배치할 캐릭터 데이터입니다. 플레이어용 CharacterDefinition도 적으로 배치할 수 있습니다."))
	TObjectPtr<UFRCharacterDefinition> CharacterDefinition;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Enemy", meta = (ToolTip = "맵 로컬 좌표 기준 적 시작 위치입니다. Z는 지형 위에서 시작하도록 충분히 높게 둡니다."))
	FVector SpawnLocal = FVector(448.0f, 0.0f, 1040.0f);

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Enemy", meta = (ToolTip = "켜면 이 적이 CharacterDefinition의 특수 공격 슬롯을 사용할 수 있습니다."))
	bool bUseSpecialAttack = false;
};

UCLASS(BlueprintType)
class FORTROGUE_API UFRTerrainMapDefinition : public UPrimaryDataAsset
{
	GENERATED_BODY()

public:
	UFRTerrainMapDefinition();
	...
	TArray<FFRTerrainTextureLayer> TextureLayers;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Spawn", meta = (TitleProperty = CharacterDefinition, ToolTip = "맵에 직접 배치할 적 목록입니다. 비어 있으면 EnemySpawnLocal과 GameMode 기본 적 데이터 하나를 사용합니다."))
	TArray<FFREnemyPlacement> EnemyPlacements;
