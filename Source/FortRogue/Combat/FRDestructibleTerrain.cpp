...
AFRDestructibleTerrain::AFRDestructibleTerrain()
{
...
void AFRDestructibleTerrain::OnConstruction(const FTransform& Transform)
{
...
void AFRDestructibleTerrain::BeginPlay()
{
	Super::BeginPlay();
...
#if WITH_EDITOR
void AFRDestructibleTerrain::PostEditMove(bool bFinished)
{
...
void AFRDestructibleTerrain::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
bool AFRDestructibleTerrain::IsSolidAtWorldLocation(const FVector& WorldLocation) const
{
	int32 X = 0;
	int32 Z = 0;
...
bool AFRDestructibleTerrain::FindSurfaceZAtWorldX(float WorldX, float StartWorldZ, float SearchDistance, float& OutSurfaceZ) const
{
	int32 X = 0;
...
bool AFRDestructibleTerrain::FindFirstSolidAlongWorldSegment(const FVector& StartWorldLocation, const FVector& EndWorldLocation, FVector& OutImpactLocation) const
{
...
bool AFRDestructibleTerrain::CarveCircle(const FVector& WorldLocation, float Radius)
{
...
bool AFRDestructibleTerrain::FillCircle(const FVector& WorldLocation, float Radius, uint8 TextureLayer)
{
	if (CellsX <= 0 || CellsZ <= 0 || CellSize <= 0.0f || Radius <= 0.0f)
	{
		return false;
...
bool AFRDestructibleTerrain::IsProjectileOutOfBounds(const FVector& WorldLocation) const
{
...
float AFRDestructibleTerrain::GetSurfaceZ() const
{
	return GetActorLocation().Z + Height;
}

FVector AFRDestructibleTerrain::GetPlayerSpawnWorldLocation() const
{
...
FVector AFRDestructibleTerrain::GetEnemySpawnWorldLocation() const
{
...
FVector AFRDestructibleTerrain::ResolveMapLocalSpawnWorldLocation(const FVector& PreferredLocalLocation, const FVector& FallbackLocalLocation) const
{
	return ResolveSpawnWorldLocation(PreferredLocalLocation, FallbackLocalLocation);
}

UTexture2D* AFRDestructibleTerrain::GetRuntimeTerrainTexture() const
{
	return RuntimeTerrainTexture.Get();
}

void AFRDestructibleTerrain::InitializeMask()
{
	if (MapDefinition)
	{
...
void AFRDestructibleTerrain::InitializeGeneratedMask()
{
...
void AFRDestructibleTerrain::InitializeMaskFromDefinition()
{
...
void AFRDestructibleTerrain::ApplyDefinitionDimensions()
{
...
void AFRDestructibleTerrain::NormalizeActorTransformForGameplayPlane()
{
...
void AFRDestructibleTerrain::ConfigureTexturePlane()
{
...
void AFRDestructibleTerrain::RebuildVisuals()
{
...
void AFRDestructibleTerrain::InitializeRuntimeTexture()
{
	CacheLayerTextures();
...
void AFRDestructibleTerrain::CacheLayerTextures()
{
...
FVector AFRDestructibleTerrain::ResolveSpawnWorldLocation(const FVector& PreferredLocalLocation, const FVector& FallbackLocalLocation) const
{
...
void AFRDestructibleTerrain::UpdateRuntimeTexture()
{
...
void AFRDestructibleTerrain::UpdateRuntimeTextureRegion(int32 MinX, int32 MinZ, int32 MaxX, int32 MaxZ)
{
...
FColor AFRDestructibleTerrain::GetTerrainPixelColor(int32 X, int32 Z) const
{
	const int32 Index = ToIndex(X, Z);
	if (!SolidMask.IsValidIndex(Index) || SolidMask[Index] == 0)
	{
...
bool AFRDestructibleTerrain::TryGetLayerTextureColor(uint8 LayerIndex, int32 X, int32 Z, FColor& OutColor) const
{
...
int32 AFRDestructibleTerrain::ToIndex(int32 X, int32 Z) const
{
	return Z * CellsX + X;
}

float AFRDestructibleTerrain::GetGeneratedSurfaceZ(int32 X) const
{
...
bool AFRDestructibleTerrain::WorldToCell(const FVector& WorldLocation, int32& OutX, int32& OutZ) const
{
	const FVector LocalLocation = WorldLocation - GetActorLocation();
...
bool AFRDestructibleTerrain::WorldXToCell(float WorldX, int32& OutX) const
{
