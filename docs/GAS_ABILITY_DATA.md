...
기본 로드아웃을 검수할 때는 `UFortRogueDefaultLoadoutDefinition::GetDataValidationSummary()` 또는 `UFortRogueRewardBlueprintLibrary::GetDefaultLoadoutDataValidationSummary()`를 사용한다. 빈 무기 목록, 비어 있는 무기/아이템 항목, 0 이하 아이템 충전 수, 내부 무기/아이템 데이터 경고를 한 줄로 보여준다.
...
보상 데이터를 검수할 때는 `RewardChoice.GetDataValidationSummary()` 또는 UMG의 `UFortRogueRewardBlueprintLibrary::GetRewardDataValidationSummary()`를 사용한다. 표시 이름 누락, 0 이하 가중치, `bOfferOncePerRun`인데 `RewardTag`가 없는 경우, 실제 gameplay effect가 없는 경우, Required/Blocked 보상 태그 겹침, 직접 붙인 `ShotModifiers` 내부 경고를 한 줄로 보여준다.
...
ShotModifier 배열만 따로 표시해야 하는 디버그/편집 UI에서는 `GetShotModifierEffectSummary()`를 사용한다. 요약에는 `ModifierTag`, `EffectTags`, 유효한 `ProjectileEffects` 개수와 EffectClass 표시명도 포함되어 제거 가능한 효과와 실제 탄 효과 태그를 바로 확인할 수 있다.
