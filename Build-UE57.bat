...
set "UE57_BUILD_BAT=D:\Program Files\Epic Games\UE_5.7\Engine\Build\BatchFiles\Build.bat"
...
if not exist "%UE57_BUILD_BAT%" (
    echo UE 5.7 Build.bat not found:
    echo "%UE57_BUILD_BAT%"
    exit /b 1
)
...
if /I "%COMMAND_MODE%"=="raw" (
    call "%UE57_BUILD_BAT%" %EXTRA_ARGS%
    exit /b %ERRORLEVEL%
)

if /I "%COMMAND_MODE%"=="print" (
    echo "%UE57_BUILD_BAT%" -Target="%BUILD_TARGET%" -WaitMutex -FromMsBuild -architecture=x64%EXTRA_ARGS%
    exit /b 0
)
